...
enStratusは、ソフトとハードクォータを持つことができる課金コードの概念を持っています。
ソフトクォータは、それが割り当てに達したとき、しかしそのコードがそれに対してまだ引き受けることが
できるときに警告が表示されます。
ハードクォータはまた、警告が発しますが、もはやそれ以上インフラやリソースが、そのコードに対して
引き受けることができない量に達してしまったときです。
...
我々はソフトとハード両方のクォータが警告メッセージを発するいくつかの例だけではなく、
課金コードに関連するいくつかの定義を見ましたし、
我々はどのように課金コードが適用されるかの周辺の機能を見ました。
我々は、課金コードが無効になるとなにが起きるかだけでなく課金コードの作成および編集を説明しました。
また、請求書閲覧を見ました。
...
最後に、我々はクラウドリソースの使用状況に関する情報をレポートの形式で取得するための非常に
きめ細かな方法を見ました。レポートを使用することにより、請求コード、リソース、ユーザー、およびグループに基づいて
使用量のフィルタリングおよびソートすることが可能です。
レポートはどのようにクラウドリソースが使用されているかを学ぶ非常に強力な方法です。
